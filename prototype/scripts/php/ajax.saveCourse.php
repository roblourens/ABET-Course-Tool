...
for($i = 0 ; $i < $data['assignment_row_count'] ; $i++){
	$course->assignments[$i]->assignment_type = $data['assignment_type_'.$i];
	$course->assignments[$i]->assignment_number = $data['assignment_number_'.$i];
	$course->assignments[$i]->learningOutcomes = array(); 

    foreach (array('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k') as $letter)
        if ($data['checkbox'.strtoupper($letter).'_'.$i] == 'on')
